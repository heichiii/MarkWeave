...
test('目录位于物理第二页，排除总标题，计章计节并在续页及无标题页继承',async()=>{
 const md=parser('.');
 ...
 try {
  ...
  assert.equal(await page.locator('.slide').nth(1).locator(':scope > h1').textContent(),'目录');
  ...
  const pages=await page.locator('.slide:not([data-toc])').evaluateAll(nodes=>nodes.map(p=>({title:p.querySelector('h1').textContent,label:p.querySelector('header').textContent})));
  assert.equal(pages[0].label,'');
  for(const p of pages.filter(p=>p.title.startsWith('背景')))assert.equal(p.label,'第 1 章 · 起步 / 第 1.1 节 · 背景');
  assert.equal(pages.find(p=>p.title==='导读').label,'第 1 章 · 起步 / 第 1.1 节 · 背景');
  assert.equal(pages.at(-1).label,'第 2 章 · 结果 / 第 2.1 节 · 结论');
  assert.equal(await page.locator('.content').evaluateAll(nodes=>nodes.some(e=>e.scrollHeight>e.clientHeight+1)),false);
...
test('超长目录保留所有条目，Logo 和目录续页排版可用',async()=>{
 ...
 try {
  ...
  assert.equal(await page.locator('.slide').nth(1).locator(':scope > h1').textContent(),'Contents');
  assert.equal(await page.locator('.content').evaluateAll(nodes=>nodes.some(e=>e.scrollHeight>e.clientHeight+1)),false);
  await fs.mkdir('tmp/qa',{recursive:true});
