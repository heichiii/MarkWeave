...
import {validateLogo} from './logo.js';
...
export async function resolveOptions(args, cwd=process.cwd()) {
 const {values,positionals}=parseArgs({args,allowPositionals:true,options:{
  ...
  logo:{type:'string'},'logo-scale':{type:'string'},'logo-position':{type:'string'}
 }});
 ...
 if(values.config!==undefined) {
  ...
  for(const [key,value] of Object.entries(config)) {
   if(key==='logo'){validateLogo(value,{partial:true});continue;}
  ...
  if(config.logo?.path!==undefined)config.logo.path=path.resolve(path.dirname(file),config.logo.path);
 }
 ...
 if(options.logo!==undefined || ['logo','logo-scale','logo-position'].some(key=>values[key]!==undefined)) {
  options.logo={...options.logo};
  if(values.logo!==undefined)options.logo.path=path.resolve(cwd,values.logo);
  if(values['logo-scale']!==undefined)options.logo.scale=Number(values['logo-scale']);
  if(values['logo-position']!==undefined)options.logo.position=values['logo-position'];
  validateLogo(options.logo);
 }
 if(!options.input) {
