...
export function parser(dir,{math=false}={}) {
  ...
  const md=new Marked({gfm:true, renderer:{
    ...
    code(t) { const lang=(t.lang||'').split(/\s/)[0].toLowerCase(); return lang==='mermaid'?`<div class="mermaid">${esc(t.text)}</div>`:`<pre><code>${lang && hljs.getLanguage(lang) ? hljs.highlight(t.text,{language:lang}).value : esc(t.text)}</code></pre>`; },
    image(t) {
...
const css=`*{box-sizing:border-box}body{margin:0;background:#eef2f6;color:#192b3c;font:16px/1.7 "Segoe UI","Microsoft YaHei",sans-serif}main{background:white;max-width:900px;margin:40px auto;padding:48px 64px;border-radius:12px}h1,h2,h3,h4{line-height:1.25;color:#123a50}h1{font-size:36px}h2{font-size:28px;border-bottom:1px solid #dce6ed;padding-bottom:12px}h3{font-size:22px}a{color:#087f8c}img{max-width:100%;max-height:380px;object-fit:contain}pre{white-space:pre-wrap;overflow-wrap:anywhere;background:#edf3f6;padding:18px;border-radius:8px;font-size:.85em}code{font-family:Consolas,monospace}blockquote{border-left:4px solid #12a395;margin:20px 0;padding:8px 20px;background:#effaf7}table{border-collapse:collapse;width:100%}td,th{border:1px solid #d6e1e8;padding:8px 12px;text-align:left}th{background:#edf5f7}p,li{overflow-wrap:anywhere}.mermaid{text-align:center;margin:20px 0}.mermaid svg{display:block;max-width:100%;height:auto;margin:auto}.hljs-keyword,.hljs-selector-tag{color:#a626a4}.hljs-string{color:#287b45}.hljs-number,.hljs-literal{color:#b45b15}.hljs-comment{color:#718096}nav{position:fixed;bottom:12px;left:50%;transform:translateX(-50%);z-index:10;background:#18394d;color:white;padding:8px 16px;border-radius:24px;display:flex;align-items:center;gap:12px}button{border:0;border-radius:16px;padding:7px 12px;cursor:pointer}body.slides main{max-width:none;padding:0;background:none;margin:0}:is(.slide,.slide-thumbnail-preview){width:1280px;height:720px;padding:48px 64px;background:white;position:relative;margin:24px auto;overflow:hidden}:is(.slide,.slide-thumbnail-preview) .content{height:558px;overflow:hidden;font-size:24px;line-height:1.5}:is(.slide,.slide-thumbnail-preview) h1{font-size:42px;margin-top:0;margin-bottom:16px}:is(.slide,.slide-thumbnail-preview) h2{font-size:32px}:is(.slide,.slide-thumbnail-preview) footer{position:absolute;bottom:22px;left:64px;color:#6c8496;font-size:14px}:is(.slide,.slide-thumbnail-preview).cover{border-top:12px solid #0b968a}:is(.slide,.slide-thumbnail-preview).cover h1{font-size:56px}:is(.slide,.slide-thumbnail-preview) img{max-height:380px}:is(.slide,.slide-thumbnail-preview) p{margin:12px 0}:is(.slide,.slide-thumbnail-preview) pre{font-size:18px}:is(.slide,.slide-thumbnail-preview) table{font-size:20px}:is(.slide,.slide-thumbnail-preview) .mermaid{margin:8px 0}:is(.slide,.slide-thumbnail-preview) .mermaid svg{max-height:470px}body.present{overflow:hidden;background:#142c3d}body.present .slide{display:none;margin:0;position:absolute;left:50%;top:50%;transform:translate(-50%,-50%) scale(var(--scale,1))}body.present .slide.active{display:block}body.flow main{max-width:none;margin:0;padding:0;background:#f4f7fb;border-radius:0}#viewport{height:100vh;overflow:hidden;touch-action:none;cursor:grab}#graph{transform-origin:0 0}svg text{font-family:"Microsoft YaHei","Segoe UI",sans-serif}@media print{body{background:white}nav{display:none}main{margin:0;padding:0;max-width:none}.slide{margin:0!important;break-after:page;display:block!important;position:relative!important;transform:none!important;left:auto!important;top:auto!important}body.present{overflow:visible}pre,blockquote,tr,img,.mermaid{break-inside:avoid}h1,h2,h3{break-after:avoid}}`;
export function wrap(title,mode,body,script='',language='zh-CN',filename=title) { messages(language);return `<!doctype html><html lang="${language}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)}</title><style>${css}${mode==='slides'?slideCss+mathCss()+'.katex-display{max-width:100%;overflow-x:auto;overflow-y:hidden}.katex{overflow-wrap:normal}':''}</style></head><body class="${mode}" data-filename="${esc(filename)}">${body}<script>${mermaidScript(body)}${script}</script></body></html>`; }
