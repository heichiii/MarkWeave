...
幻灯片自动在每页顶部添加章节标注，并在物理第二页插入目录（过长目录自动续页）。第一页的第一个一级标题视为总标题，不计入章数；之后每个一级标题计为新章，二级标题计为节，每章内从 1 重新编号，例如 `1.1`、`1.2`、`2.1`。三级及更深标题、代码块和引用内部的标题不参与统计。没有所属章的二级标题按独立节编号。目录仅按原文顺序列出章标题，采用大字号单列布局；节标题和封面不列入目录。

页头显示该页起始内容所属的章节，无标题页及自动续页继承上下文；页内出现后续章节时不会强制切页，后续页会使用新的章节上下文。封面和目录页不显示顶部的小字标注；正文页继续显示所属章节。Logo 与章节标注各自预留空间。空文档不生成目录。
